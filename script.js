...
function showDelSkema(delNumber) {
  try {
    // Hide all DEL skemaer
    for (let i = 1; i <= 4; i++) {
      const delSkema = document.getElementById(`del${i}-skema`);
      if (delSkema) {
        delSkema.style.display = 'none';
      }
    }
    
    // Show selected DEL skema
    const selectedDelSkema = document.getElementById(`del${delNumber}-skema`);
    if (selectedDelSkema) {
      selectedDelSkema.style.display = 'block';
    }
  } catch (error) {
    console.error('Error in showDelSkema:', error);
  }
}
...
function generateDelSkema(delNumber, weekEvents) {
  try {
    ...
    const weekActivities = [];
      ...
      // Add events for multiple sergeants per day
      for (let day = 1; day <= 5; day++) {
        const dayIndex = day - 1;
        const primarySergeant = delSergeants[dayIndex] || 'SGT';
        const secondarySergeant = delSergeants[(dayIndex + 1) % delSergeants.length] || 'SGT';
        
        // Add activities with different sergeants
        const multiSergeantActivities = [
          { activity: 'Fysisk træning', time: '0800-0900', leader: primarySergeant, color: '#FFEAA7' },
          { activity: 'BM-appel', time: '0900-0915', leader: primarySergeant, color: '#34495e' },
          { activity: 'Instruktion', time: '0915-1100', leader: secondarySergeant, color: '#4ECDC4' },
          { activity: 'Frokost', time: '1200-1300', leader: primarySergeant, color: '#f8f9fa' },
          { activity: 'Øvelse', time: '1300-1500', leader: secondarySergeant, color: '#FF6B6B' },
          { activity: 'Efterarbejde', time: '1500-1600', leader: primarySergeant, color: '#45B7D1' }
        ];

        multiSergeantActivities.forEach(item => {
          weekActivities.push({
            date: dayNames[day],
            time: item.time,
            enh: `${delNumber}. DEL`,
    ...
    weekActivities.sort((a, b) => {
      const dayOrder = { 'MAN': 1, 'TIR': 2, 'ONS': 3, 'TOR': 4, 'FRE': 5 };
      const dayDiff = dayOrder[a.date] - dayOrder[b.date];
    ...
    // Add week summary header row
    const weekSummaryRow = document.createElement('tr');
    weekSummaryRow.style.backgroundColor = '#e3f2fd';
    weekSummaryRow.style.borderTop = '3px solid #2196f3';
    weekSummaryRow.style.borderBottom = '2px solid #2196f3';
    
    // DATO - Week summary
    const dateSummaryCell = document.createElement('td');
    dateSummaryCell.textContent = 'UGEN';
    dateSummaryCell.style.fontWeight = 'bold';
    dateSummaryCell.style.fontSize = '14px';
    dateSummaryCell.style.padding = '12px 8px';
    dateSummaryCell.style.backgroundColor = '#e3f2fd';
    weekSummaryRow.appendChild(dateSummaryCell);

    // TID - Empty
    const timeSummaryCell = document.createElement('td');
    timeSummaryCell.textContent = '';
    weekSummaryRow.appendChild(timeSummaryCell);

    // ENH - DEL number
    const enhSummaryCell = document.createElement('td');
    enhSummaryCell.textContent = `${delNumber}. DEL`;
    enhSummaryCell.style.fontWeight = 'bold';
    weekSummaryRow.appendChild(enhSummaryCell);

    // AKTIVITET - Sergeants for the week
    const activitySummaryCell = document.createElement('td');
    const hoursText = delSergeants.map(sgt => `${sgt} = ${sergeantHours[sgt]} timer`).join(', ');
    
    activitySummaryCell.textContent = hoursText;
    activitySummaryCell.style.fontWeight = 'bold';
    activitySummaryCell.style.color = '#1565c0';
    activitySummaryCell.style.fontSize = '13px';
    weekSummaryRow.appendChild(activitySummaryCell);

    // LEDER - Empty
    const leaderSummaryCell = document.createElement('td');
    leaderSummaryCell.textContent = '';
    weekSummaryRow.appendChild(leaderSummaryCell);

    // PÅKL. - Empty
    const dressSummaryCell = document.createElement('td');
    dressSummaryCell.textContent = '';
    weekSummaryRow.appendChild(dressSummaryCell);

    // STED - Empty
    const locationSummaryCell = document.createElement('td');
    locationSummaryCell.textContent = '';
    weekSummaryRow.appendChild(locationSummaryCell);

    tbody.appendChild(weekSummaryRow);

    // Generate table rows with grouped days
    Object.keys(activitiesByDay).forEach(day => {
      ...
      dayHeaderRow.style.borderTop = '2px solid #dee2e6';
      
      // DATO - Day name
      const dateHeaderCell = document.createElement('td');
      dateHeaderCell.textContent = day;
      dateHeaderCell.style.fontWeight = 'bold';
      dateHeaderCell.style.fontSize = '14px';
      dateHeaderCell.style.padding = '12px 8px';
      dateHeaderCell.style.borderBottom = '1px solid #dee2e6';
      dayHeaderRow.appendChild(dateHeaderCell);

      // TID - Empty
      const timeHeaderCell = document.createElement('td');
      timeHeaderCell.textContent = '';
      dayHeaderRow.appendChild(timeHeaderCell);

      // ENH - DEL number
      const enhHeaderCell = document.createElement('td');
      enhHeaderCell.textContent = `${delNumber}. DEL`;
      enhHeaderCell.style.fontWeight = 'bold';
      dayHeaderRow.appendChild(enhHeaderCell);

      // AKTIVITET - Empty (sergeants shown in week summary)
      const activityHeaderCell = document.createElement('td');
      activityHeaderCell.textContent = '';
      dayHeaderRow.appendChild(activityHeaderCell);

      // LEDER - Empty
      const leaderHeaderCell = document.createElement('td');
      leaderHeaderCell.textContent = '';
      dayHeaderRow.appendChild(leaderHeaderCell);

      // PÅKL. - Empty
      const dressHeaderCell = document.createElement('td');
      dressHeaderCell.textContent = '';
      dayHeaderRow.appendChild(dressHeaderCell);

      // STED - Empty
      const locationHeaderCell = document.createElement('td');
      locationHeaderCell.textContent = '';
      dayHeaderRow.appendChild(locationHeaderCell);
      ...
      dayActivities.forEach(activity => {
        ...
        // DATO - Empty (already shown in header)
        ...
        // ENH - Empty (already shown in header)
        ...
        if (activity.backgroundColor) {
          activityCell.style.backgroundColor = activity.backgroundColor;
          activityCell.style.color = '#fff';
        ...
        leaderCell.textContent = activity.leader;
        row.appendChild(leaderCell);

        // PÅKL.
        const dressCell = document.createElement('td');
        dressCell.textContent = activity.dress;
        row.appendChild(dressCell);
    ...
    console.error(`Error generating skema for DEL ${delNumber}:`, error);
  }
}
